...
This script queries repositories that are missing pushed_at dates and fetches
the data from GitHub's API to populate the field.
"""
...
def backfill_pushed_at():
    """Backfill missing pushed_at dates for all repositories."""
    engine = create_engine(DATABASE_URL)
    Session = sessionmaker(bind=engine)
    db = Session()

    try:
        # Get all repositories missing pushed_at
        ...
        logger.info(f"Found {total} repositories missing pushed_at dates")
        logger.info(f"Starting backfill...")

        updated = 0
        skipped = 0
        errors = 0

        for idx, (repo_id, repo_name, full_name, org_name) in enumerate(repos, 1):
            logger.info(f"[{idx}/{total}] Processing: {full_name or f'{org_name}/{repo_name}'}")
            ...
            if pushed_at:
                ...
                logger.info(f"  ✓ Updated pushed_at: {pushed_at}")
                updated += 1
            else:

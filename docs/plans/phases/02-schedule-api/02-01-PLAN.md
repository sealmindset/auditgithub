# Phase 2 Plan 1: Schedule API

**Backend REST endpoints for schedule CRUD operations**

## Objective

Create a new FastAPI router for scan schedule management with full CRUD operations, following existing patterns from `repositories.py` and `scans.py`. The router will provide endpoints for listing, retrieving, updating schedules, and managing manual overrides with audit logging.

## Execution Context

```
@src/api/routers/schedules.py (create)
@src/api/main.py (modify - add router)
@src/api/models.py (reference - ScanSchedule, ScheduleOverride)
```

## Context

### From Phase 1
- `ScanSchedule` model with: schedule_type, frequency, day_of_week, time_window, scan_arguments, is_locked
- `ScheduleOverride` model for audit trail tracking previous/new values
...
### Existing Patterns
- Routers use `APIRouter(prefix="/path", tags=["tag"])`
- Dependency injection: `get_tenant_db`, `get_current_user`, `require_permissions`
- Pydantic schemas defined at top of router file
- UUID IDs converted to strings for responses
...
- Override create: `schedules:override`

## Tasks

### Task 1: Create schedules router with Pydantic schemas

**File:** `src/api/routers/schedules.py`

Create the router file with all Pydantic request/response models:
...
**Verification:**
- File exists at `src/api/routers/schedules.py`
- Imports compile without errors
- All enums match database CHECK constraints

---

### Task 2: Implement GET endpoints (list and single)

**File:** `src/api/routers/schedules.py`

Add the read endpoints after the schema definitions:
...
**Verification:**
- GET /schedules returns list with total count
- GET /schedules/{repo_id} returns single schedule
- Both include repository_name and locked_by_email

---

### Task 3: Implement PUT endpoint for schedule updates
...
**Verification:**
- PUT /schedules/{repo_id} updates schedule
- Creates ScheduleOverride audit record
- Sets schedule_type to "manual" and is_locked to True

---

### Task 4: Implement override management endpoints

**File:** `src/api/routers/schedules.py`

Add lock/unlock and history endpoints:
...
**Verification:**
- POST /schedules/{repo_id}/lock locks without changing values
- DELETE /schedules/{repo_id}/lock unlocks and returns to AI management
- GET /schedules/{repo_id}/history returns audit trail
...
Add import and include_router for schedules:

1. Add import near other router imports (around line 30):
```python
from .routers import schedules
```

2. Add include_router after scheduler.router (around line 163):
```python
...
- `/docs` shows schedules endpoints
- Endpoints respond (may return empty data)

## Verification

After all tasks complete:

1. **Import check**: `python -c "from src.api.routers import schedules"`
2. **Syntax check**: `python -m py_compile src/api/routers/schedules.py`
3. **API startup**: Verify no import errors when starting the API

## Success Criteria

- [ ] New router file `src/api/routers/schedules.py` exists
- [ ] All 6 endpoints implemented:
  - GET /schedules (list)
  - GET /schedules/{repo_id} (single)
  - PUT /schedules/{repo_id} (update)
  - POST /schedules/{repo_id}/lock (lock)
  - DELETE /schedules/{repo_id}/lock (unlock)
  - GET /schedules/{repo_id}/history (audit)
- [ ] Pydantic schemas with proper validation
- [ ] ScheduleOverride records created on changes

...
import { Loader2, RefreshCw, Save, Edit, X, ImagePlus, FileEdit, Wand2, History, GitBranch, Brain } from "lucide-react"
...
export function ArchitectureView({ projectId, organization, repositoryName }: ArchitectureViewProps) {
    ...
    useEffect(() => {
        const fetchArchitecture = async () => {
            try {
                const res = await apiFetch(`${API_BASE}/ai/architecture/${projectId}`)
                if (res.ok) {
                    const data = await res.json()
    ...
    const generateArchitecture = async () => {
        ...
        try {
            const res = await apiFetch(`${API_BASE}/ai/architecture`, {
                method: "POST",
    ...
    const fetchVersions = async () => {
        try {
            const res = await apiFetch(`${API_BASE}/ai/architecture/${projectId}/versions`)
            if (res.ok) {
    ...
    const restoreVersion = async (versionId: string) => {
        ...
        try {
            const res = await apiFetch(`${API_BASE}/ai/architecture/${projectId}/restore/${versionId}`, {
                method: "POST"
            })
            ...
            const archRes = await apiFetch(`${API_BASE}/ai/architecture/${projectId}`)
            if (archRes.ok) {
        ...
        <div className="space-y-6">
            ...
            <Tabs defaultValue="diagram" className="w-full">
                <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                        {diagramImage && organization && !editMode && (
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={async () => {
                                    try {
                                        const res = await apiFetch(`${API_BASE}/git-sync/push-diagram`, {
                                            method: "POST",
                                            headers: { "Content-Type": "application/json" },
                                            body: JSON.stringify({
                                        ...
                                        link.download = `${repositoryName || projectId}.png`
                                        link.click()
                                    }
                                }}
                                disabled={loading || saving}
                                title="Push diagram to repository"
                            >
                                <GitBranch className="mr-2 h-4 w-4" />
                                Git PNG
                            </Button>
                        )}
                        {report && organization && !editMode && (
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={async () => {
                                    ...
                                    try {
                                        const res = await apiFetch(`${API_BASE}/git-sync/push-readme`, {
                                            method: "POST",
                                            headers: { "Content-Type": "application/json" },
                                            body: JSON.stringify({
                                                project_id: projectId,
                                        ...
                                        console.error("Git README error", err)
                                        toast({
                                            title: "Error",
                                ...
                                disabled={loading || saving}
                                title="Push report to README.md"
                            >
                                <GitBranch className="mr-2 h-4 w-4" />
                                Git README
                            </Button>
                            ...
                            <>
                                <Button
                                    ...
                                    onClick={async () => {
                                        setLoading(true)
                                        try {
                                            const res = await apiFetch(`${API_BASE}/ai/architecture/refine`, {
                                                method: "POST",
                                                headers: { "Content-Type": "application/json" },
            ...
            <AskAIModal
                ...
                repositoryId={0} // TODO: Get actual repository ID from props
                repositoryName={repositoryName || "Repository"}
                context={{
                    technicalOverview: report,
                    scanResults: [],
                    vulnerabilities: [],
                }}
            />

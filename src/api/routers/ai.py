...
class ZeroDayResponse(BaseModel):
    ...
    hunt_evidence: Optional[Dict[str, Any]] = Field(
        None,
        description="Raw output of each threat-hunting tool that ran: registry ground "
                    "truth, source arbitration, CI activity, endpoint telemetry, alerts."
    )
    coverage_notes: Optional[List[str]] = Field(
        None,
        description="Every limit on what this run could see — member-level tokens, "
                    "missing Graph roles, failed tools, retention windows. Returned "
                    "separately from the prose so a caller cannot mistake an unexamined "
                    "surface for a clean one."
    )
    hunt_enabled: Optional[bool] = Field(
        None,
        description="False when the caller lacks hunt:execute, in which case only local "
                    "database tools ran and no external evidence was gathered."
    )
...
async def analyze_zero_day(
    ...
    db: Session = Depends(get_tenant_db),
    can_hunt: bool = Depends(check_permission("hunt:execute"))
):
    """
    Run a zero-day / supply-chain hunt.

    Searches local scan data for affected dependencies, findings and technologies.
    **Required permissions:** findings:write.

    Callers who additionally hold **hunt:execute** get the live evidence surfaces:
    package-registry ground truth, cross-source arbitration, CI/CD activity in the
    attack window, Defender XDR endpoint telemetry and unified alerts. Without it the
    analysis still runs, but every external tool in the plan is recorded in
    `coverage_notes` as an unexamined surface rather than being dropped — an absence the
    hunt was not permitted to test is not a clean result.
    """
    if not ai_agent:
        raise HTTPException(status_code=503, detail="AI Agent not initialized")
...
async def get_zero_day_prompt():
    """
    Get the zero-day analysis planning prompt currently in use.

    Read from src.ai_agent.zda_prompt, the same module reasoning.py fills per request,
    so this endpoint cannot drift from the live prompt. It previously returned its own
    ...
    return {
        "prompt": zda_prompt.planning_prompt_template(),
        "synthesis_prompt": zda_prompt.SYNTHESIS_PROMPT,
        "tools": zda_prompt.tool_specs(),
        "hunt_tools_requiring_permission": sorted(zda_prompt.HUNT_TOOL_NAMES),
        "placeholders": ["query", "scope_str", "organizations"],
    }
...
async def validate_zero_day_prompt(
    ...
    try:
        ...
        formatted_prompt = (request.prompt
                            .replace("{query}", test_query)
                            .replace("{scope_str}", scope_str)
                            .replace("{organizations}", "sleepnumber, sleepnumberlabs, SleepNumberInc")
                            .replace("{tool_catalog}", zda_prompt.render_tool_catalog()))
        ...
        if hasattr(ai_agent.reasoning_engine.provider, "execute_prompt"):
            ...
            # Parsing as JSON is not validation. A prompt that reliably plans tools the
            # dispatcher cannot call produces a plan that executes nothing, which the
            # old check reported as success.
            known = set(zda_prompt.tool_names())
            planned = [t.get("name") for t in plan.get("tools", [])]
            unknown = sorted({n for n in planned if n not in known})
            hunt_used = sorted({n for n in planned if n in zda_prompt.HUNT_TOOL_NAMES})

            body = (f"**Test Query:** {test_query}\n\n"
                    f"**AI Plan:**\n```json\n{json.dumps(plan, indent=2)}\n```\n\n"
                    f"**Thought:** {plan.get('thought', 'N/A')}\n\n"
                    f"**Tools to Execute:** {len(planned)}\n\n"
                    f"**Hunt tools planned (need hunt:execute):** "
                    f"{', '.join(hunt_used) if hunt_used else 'none'}")

            if unknown:
                return {
                    "success": False,
                    "unknown_tools": unknown,
                    "response": (f"⚠️ **Prompt returns valid JSON but plans unknown tools**\n\n"
                                 f"These would be skipped at execution: "
                                 f"{', '.join(unknown)}\n\n"
...
def _export_str(value, default: str = "") -> str:
    """
    Coerce an export payload value to a string.

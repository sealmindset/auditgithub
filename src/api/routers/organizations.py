...
class ImportSingleRepoRequest(BaseModel):
    """Request model for importing a single repository."""
    repo_name: str = Field(..., description="Repository name to import from GitHub")
    auto_scan: bool = Field(False, description="Automatically start a scan after import")


@router.post(
    "/{org_name}/import-repo",
    summary="Import a single repository from GitHub",
    responses={**CREATE_ERRORS, 404: {"description": "Organization or repository not found"}, 502: {"description": "GitHub API returned an error"}},
)
async def import_single_repository(
    org_name: str,
    request: ImportSingleRepoRequest,
    db: Session = Depends(get_tenant_db)
):
    """
    Import a single repository from GitHub by name.

    Fetches the repository metadata from the GitHub API and creates or updates
    it in the database. Optionally triggers a security scan after import.
    """
    ...
    try:
        ...
        github_org = org.github_org
        if not github_token:
            raise HTTPException(status_code=400, detail=f"GitHub token not configured for '{org_name}'")
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to retrieve credentials: {str(e)}")
    ...
    try:
        url = f"https://api.github.com/repos/{github_org}/{request.repo_name}"
        response = http_requests.get(url, headers=headers, timeout=30)
        response.raise_for_status()
        github_repo = response.json()
    except http_requests.exceptions.HTTPError as e:
        if e.response.status_code == 404:
            raise HTTPException(status_code=404, detail=f"Repository '{github_org}/{request.repo_name}' not found on GitHub")
        elif e.response.status_code == 401:
            raise HTTPException(status_code=401, detail="Invalid GitHub token")
        else:
            raise HTTPException(status_code=502, detail=f"GitHub API error: {str(e)}")
    ...
    action = "updated" if existing_repo else "created"
    repo = existing_repo or models.Repository(organization_id=org.id, name=repo_name)
    ...
    if request.auto_scan:
        try:
            agent = await ensure_agent_initialized()
            await agent.start_scan(org_name, repos=[repo_name], scan_type="full")
            result["scan_started"] = True
        except Exception as scan_err:
            result["scan_error"] = str(scan_err)
...
@router.get(
    ...
    summary="Search GitHub repos in the organization",
    responses={**LIST_ERRORS, 404: {"description": "Organization not found"}},
)
async def search_github_repos(
    ...
    """
    Search for repositories on GitHub within the organization.

    Returns matching repos with a flag indicating whether each is already imported.
    """
    import requests as http_requests
    ...
    try:
        ...
        manager = get_secrets_manager()
        github_token = await manager.get_secret(f"{org_name.lower()}/github_token")
        github_org_name = org.github_org
        if not github_token:
            raise HTTPException(status_code=400, detail=f"GitHub token not configured for '{org_name}'")
    except HTTPException:
        raise
    ...
    try:
        ...
        params = {"q": f"{q} org:{github_org_name}", "per_page": 20, "sort": "updated"}
        response = http_requests.get(search_url, headers=headers, params=params, timeout=30)
        response.raise_for_status()
        data = response.json()
    except http_requests.exceptions.HTTPError as e:
    ...
    imported_names = set(
        r.name for r in db.query(models.Repository.name).filter(
            models.Repository.organization_id == org.id
        ).all()
    )
    ...
    for item in data.get("items", []):
        results.append({
            ...
            "already_imported": item["name"] in imported_names,
        })

    return {"results": results, "total": data.get("total_count", 0)}
